...
- `1.6 HIGH`: Deploy `api` unhealthy — alembic.ini usava URL hardcoded (`localhost`) e `-x` no entrypoint estava com sintaxe errada. Corrigido: `alembic.ini` usa `sqlalchemy.url = ${DATABASE_URL}` (var env), migrations idempotentes com `if_not_exists=True`.
